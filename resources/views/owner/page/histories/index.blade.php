...
<style>
    .history-table-wrap .dataTables_wrapper { color: inherit; }
    ...
    .history-table-wrap .dataTables_filter input { min-width: 14rem; }
    .history-table-wrap table.dataTable thead th { border-bottom: 1px solid #374151 !important; }
    .history-table-wrap table.dataTable.no-footer { border-bottom: none !important; }
    .history-badge {
        display: inline-flex;
        align-items: center;
        border-radius: 999px;
    ...
    .history-badge-transfer { background: rgba(59, 130, 246, 0.18); color: #93C5FD; }
    .history-badge-check { background: rgba(16, 185, 129, 0.18); color: #6EE7B7; }
    .history-badge-uncheck { background: rgba(248, 113, 113, 0.18); color: #FCA5A5; }
    .history-table-wrap .dataTables_wrapper .dataTables_paginate {
        display: flex;
        align-items: center;
    ...
    .history-filters > * { min-height: 2.75rem; }
    .history-filters .history-filter-search { width: 100%; }
    ...
    @media (max-width: 768px) {
        ...
        .history-table-wrap .dataTables_wrapper .dataTables_filter input { min-width: 100%; }
    }
</style>
...
<div class="mt-8">
    <div class="flex flex-col gap-2 md:flex-row md:items-end md:justify-between mb-6">
        <div>
            <h2 class="text-2xl font-semibold text-gray-700 dark:text-gray-200">Riwayat Transaksi</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400">Lihat semua transaksi transfer dan aktivitas checklist setoran dari seluruh sistem.</p>
        </div>

        <form method="GET" action="{{ route('owner.transaction-histories.index') }}" class="w-full">
            <div class="history-filters">
                <input
                    type="text"
                    name="search"
                    value="{{ $search ?? '' }}"
                    placeholder="Cari nama pengguna atau staff..."
                ...
                <button type="submit" class="history-filter-button px-5 py-2.5 text-sm font-semibold text-white bg-green-600 rounded-xl hover:bg-green-700">
                    Cari
                </button>
                <a href="{{ route('owner.transaction-histories.index') }}" class="history-filter-link px-5 py-2.5 text-sm font-semibold text-gray-700 bg-gray-200 rounded-xl hover:bg-gray-300 dark:text-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600">
                    Reset
                </a>
            </div>
        </form>
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs bg-white dark:bg-gray-800 history-table-wrap">
        <div class="w-full overflow-x-auto">
            <table id="owner-transaction-history-table" class="w-full whitespace-no-wrap">
                <thead>
