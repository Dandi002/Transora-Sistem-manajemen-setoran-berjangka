...
<div class="w-full max-w-3xl px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
    ...
    <form action="{{ route($routePrefix . '.saving-plans.update', $savingPlan) }}" method="POST">
        @csrf
        @method('PUT')

        <label class="block text-sm">
            <span class="text-gray-700 dark:text-gray-400">Nama Paket</span>
            <input type="text" name="name" required value="{{ old('name', $savingPlan->name) }}"
        ...
        <label class="block mt-4 text-sm">
            ...
            <input type="number" min="1000" step="1000" name="weekly_amount" required value="{{ old('weekly_amount', $savingPlan->weekly_amount) }}"
                   class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700
                          focus:border-purple-400 focus:outline-none focus:shadow-outline-purple
                          dark:text-gray-300 dark:focus:shadow-outline-gray form-input" />
        </label>
        ...
        <label class="inline-flex items-center mt-4">
            <input type="checkbox" name="is_active" value="1" class="form-checkbox text-purple-600" {{ old('is_active', $savingPlan->is_active) ? 'checked' : '' }}>
            <span class="ml-2 text-sm text-gray-700 dark:text-gray-400">Paket aktif</span>
        </label>
